...
public class ContestService implements IContestService {
    ...
    private List<Question> pickQuestionsList(final List<Question> questions,final Integer numQuestion){
        List<Question> sufl1 = new ArrayList<>(questions);
        Collections.shuffle(sufl1);
        List<Question> ans = sufl1.subList(0, Math.min(numQuestion, sufl1.size()));
    ...
    if(level==null){
         allContests= contestRepository.findAll();
         return allContests;
    }else{
        allContests= contestRepository.findAllContestLevelWise(level);
        return allContests;

        // return allContests.stream().filter(contest->
        //     {System.out.println("Contest Level: "+contest.getLevel() + ", Target Level: " + level);
        // return contest.getLevel().equals(level);
