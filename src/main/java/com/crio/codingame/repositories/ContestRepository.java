...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        ...
        List<Contest> l1 = new ArrayList<Contest>(contestMap.values());
        List<Contest> cont=l1.stream().filter(e->e.getLevel().toString().equals(level.toString())).collect(Collectors.toList());
        return cont;
